...
class QuartzHotkeyListener(HotkeyListener):
    ...
    def start(self) -> None:
        if platform.system() != "Darwin":
            raise RuntimeError("QuartzHotkeyListener is only available on macOS")
        if not _QUARTZ_AVAILABLE:
            raise RuntimeError("Quartz framework is not available; install pyobjc-framework-Quartz")
        with self._lock:
            if self._running:
    ...
    def _cleanup(self) -> None:
        ...
        if self._run_loop_source is not None:
            try:
                if self._run_loop is not None:
                    CFRunLoopRemoveSource(self._run_loop, self._run_loop_source, 0x01)
            except Exception as e:
    ...
    def _handle_key_down(self, hotkey: str, reg: _QuartzRegistration) -> None:
        try:
    ...
    def _handle_key_up(self, hotkey: str, reg: _QuartzRegistration) -> None:
        if self._mode != "push_to_talk":
            return
        try:
